...
class CourseWorkUi : JFrame("Notes") {
    ...
    init {
        ...
        //чтение с файла
        val inputStreamTable: InputStream = File("src/main/kotlin/course_work/readTable.txt").inputStream()
        inputStreamTable.bufferedReader().forEachLine { lineList.add(it) }

        val inputStreamContent: InputStream = File("src/main/kotlin/course_work/texts.txt").inputStream()
        inputStreamContent.bufferedReader().forEachLine { content.add(it) }

        val inputStreamTask: InputStream = File("src/main/kotlin/course_work/task.txt").inputStream()
        inputStreamTask.bufferedReader().forEachLine { task.add(it) }

        val inputStreamNumberTask: InputStream = File("src/main/kotlin/course_work/numberTask.txt").inputStream()
    ...
    private fun createButtons(): Component {
        ...
        deleteButton.addActionListener {
            ...
            if (lineList[row - 3] == "To do list") {
                ...
                File("src/main/kotlin/course_work/task.txt").bufferedWriter().use { out ->
                    for (i in 0 until task.size) {
                        out.write(task[i])
                        out.write("\n")
                    }
                }
                File("src/main/kotlin/course_work/numberTask.txt").bufferedWriter().use { out ->
